...
Windows 桌面工具。按窗口标题定位 `Endfield`，离线识别弹性需求物资名称和最近 7 天价格，保存历史并计算本周未来价格。

## 运行

要求：Windows 10/11、[.NET 10 SDK](https://dotnet.microsoft.com/download/dotnet/10.0)。

双击 `start_overlay.cmd`。脚本会增量构建并启动完整物资总览，不再显示启动小悬浮窗。

使用流程：

1. 游戏使用窗口或无边框全屏模式，打开弹性需求物资页面。
2. 首次录入历史时打开单个物资详情页，点击“识别当前物资”，核对名称及从旧到新的 7 个价格后确认。
3. 每日更新时打开地区调度总览，确保购买额度栏和两排价格卡片完整可见，点击“批量识别今日价格”，核对地区、当前可买、容量上限、每日恢复及全部价格后一次写入。
4. 左侧查看全部物资，右侧趋势图显示最近 7 个有效日期及具体价格，并提示距离可预测还需记录几天。
5. 详情页识别位置变化时从标题栏“•••”进入调试功能并选择“校准 OCR 识别区域”；按 `Esc` 可立即退出。
...
- 感谢卓越行动的[《【终末地】从三万多条弹性物资价格中，我们破解了价格变动的规律》](https://www.bilibili.com/video/BV1zgRAByECH)，为价格规律分析提供参考。
- 感谢 Wangyd666 的[《【终末地】弹性物资怎么买？终末地弹性物资价格分析仪正式发布！》](https://www.bilibili.com/video/BV1ahjt6CEpZ)，为本项目提供思路与参考。
